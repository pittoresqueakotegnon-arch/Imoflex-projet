...
const Retrait: React.FC = () => {
  ...
  // Conservée pendant les retries manuels : une même intention ne peut générer
  // qu'un seul retrait côté serveur, même si la réponse réseau est perdue.
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(
        result.fedapay_payout_id
          ? 'Retrait demandé avec succès !'
          : 'Retrait en cours de vérification. Votre solde reste réservé jusqu’à confirmation.',
        'success'
      );
      ...
      // Une erreur applicative est définitive pour cette tentative. Une erreur
      // réseau conserve la clé afin qu'un retry reste strictement idempotent.
      if (!/Failed to fetch|network|réseau/i.test(message)) {
  ...
  return (
    <div
      className="min-h-screen bg-[var(--imx-bg-app)] text-[var(--imx-text-primary)] flex flex-col px-6 pb-8"
      style={{
