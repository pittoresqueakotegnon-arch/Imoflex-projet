          ...
          for (const l of leasesData as any[]) {
            if (!periodsByLease.has(l.id) && l.properties && l.properties.monthly_rent) {
              const { data: newPeriod, error: insertError } = await supabase
                .rpc('ensure_current_rent_period', { p_lease_id: l.id });
      ...
      <div className="px-5 flex-1">
        {sortedLeases.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6" style={{ background: 'var(--imx-surface-2)', border: '1px solid var(--imx-border)' }}>
              <KeyRound size={28} color="var(--imx-accent-glow)" />
            </div>
            <h2 className="font-nunito font-black text-[18px] text-[var(--imx-text-primary)] mb-3">Pas encore de logement actif</h2>
            ...
            <div className="w-full h-px bg-[var(--imx-border)] my-8"></div>

            <h3 className="text-[var(--imx-text-primary)] font-nunito font-black text-[15px] mb-5 self-start text-left w-full">En attendant, continuez votre recherche</h3>

            <button
              onClick={() => navigate('/')}
              className="w-full text-[var(--imx-accent-light)] font-bold rounded-2xl py-[18px]"
              style={{ background: 'transparent', border: '1px solid var(--imx-border)', fontFamily: 'Sora', fontSize: '15px' }}
            >
          ...
          <>
            <div className="flex items-center justify-between mb-1 mt-1">
              <h2 className="font-nunito font-black text-[var(--imx-text-primary)] text-[15px]">
                Mes logements ({sortedLeases.length})
              </h2>
              <button
                onClick={() => navigate('/rejoindre')}
                className="text-[var(--imx-accent-light)] text-[11px] font-semibold"
            ...
            {sortedLeases.length > 1 && (
              <p className="text-[var(--imx-text-secondary)] text-[11px] mb-4" style={{ fontFamily: 'Space Grotesk' }}>
            ...
            <div className="flex flex-col gap-3 mb-6">
              {sortedLeases.map((lease, index) => {
                ...
                if (index === 0 && (status === 'retard' || status === 'a_venir')) {
                  return (
                    <div key={lease.leaseId} className="loyer-hero p-5 mb-2 relative rounded-[24px]">
                      <div className="flex justify-between items-start mb-3 relative z-10">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-white/80 font-space-grotesk">Prochain Loyer</span>
                        <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold font-space-grotesk flex items-center gap-1 ${status === 'retard' ? 'bg-red-500/25 text-[#FF8080]' : 'bg-white/20 text-white'}`}>
                ...
                const statusColor = status === 'retard' ? '#EF4444' : status === 'solde' ? '#22C55E' : 'var(--imx-text-secondary)';

                return (
                  <button
                    key={lease.leaseId}
                    onClick={() => navigate(`/logement/${lease.leaseId}`)}
                    className="flex items-center gap-3 rounded-2xl px-4 py-3.5 text-left w-full transition-all active:scale-[0.99]"
                    style={{
                      background: 'var(--imx-surface)',
                      border: status === 'retard' ? '1px solid rgba(239, 68, 68, 0.4)' : '1px solid var(--imx-border)',
                      boxShadow: 'var(--imx-card-shadow-sm)',
                      opacity: status === 'solde' ? 0.75 : 1,
                    }}
                  >
                    <div className="flex-shrink-0">{statusIcon}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[var(--imx-text-primary)] font-nunito font-bold text-[14px] truncate">{lease.propertyName}</p>
                      <p className="text-[12px] truncate font-medium" style={{ color: statusColor, fontFamily: 'Space Grotesk' }}>
                        {statusText}
                      </p>
                    </div>
                    {period && (
                    ...
                    <ChevronRight size={16} color="var(--imx-text-muted)" className="flex-shrink-0" />
                  </button>
                );
              })}
            </div>
            ...
            {recentPayments.length > 0 && (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="font-nunito font-black text-[var(--imx-text-primary)] text-[15px]">Derniers versements</h2>
                  <Link to="/historique" className="text-[var(--imx-accent-light)] text-[11px] font-semibold" style={{ fontFamily: 'Space Grotesk' }}>
                    Voir tout →
                  </Link>
                </div>
