        ...
        {sortedLeases.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <button
              ...
              style={{ background: 'transparent', border: '1px solid rgba(168, 85, 247, 0.4)', fontFamily: 'Sora', fontSize: '15px' }}
            >
                  ...
                  return (
                    <div key={lease.leaseId} className="rounded-[24px] p-5 mb-2 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #261C55 0%, #1A1240 100%)', border: `1px solid ${status === 'retard' ? 'rgba(239, 68, 68, 0.4)' : 'rgba(168, 85, 247, 0.3)'}` }}>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold font-space-grotesk flex items-center gap-1 ${status === 'retard' ? 'bg-red-500/15 text-[#EF4444]' : 'bg-[var(--imx-accent-light)]/15 text-[var(--imx-accent-light)]'}`}>
                          {status === 'retard' ? <AlertCircle size={12} /> : <Clock size={12} />}
                          {status === 'retard' ? `J-${Math.abs(daysLeft)} (Retard)` : `J-${daysLeft}`}
                        </div>
                      </div>
                      
                      <p className="font-nunito font-900 text-3xl text-white leading-none mb-1">{new Intl.NumberFormat('fr-FR').format(remaining)} <span className="text-sm text-white/70 font-normal">F</span></p>
                      <p className="text-[13px] text-white/90 font-nunito font-bold mb-4">{lease.propertyName}</p>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: status === 'retard' ? '1px solid rgba(239, 68, 68, 0.35)' : '1px solid var(--imx-border)',
