...
import { calculateProrataAmount } from '../../lib/utils';
...
export default function Rejoindre() {
  ...
  const handleInputSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prévisualisation contrôlée par une RPC : le code d'accès ne doit jamais
      // être récupérable via un select direct sur la table properties.
      const { data: propData, error: propError } = await supabase
        .rpc('preview_property_by_access_code', { p_access_code: code.trim() })
        .maybeSingle();
      ...
      setProperty(propData as JoinedProperty);
  ...
  const handleConfirmation = async () => {
    ...
    try {
      // Le bail, le prorata, l'échéance et l'occupation de l'annonce sont
      // calculés dans une transaction serveur à partir du code d'accès.
      const { error: joinError } = await supabase.rpc('join_property_with_access_code', {
      ...
      if (databaseError.code === '23505' || databaseError.message?.includes('uniq_active_lease_per_property')) {
