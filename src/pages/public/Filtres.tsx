...
const FilterCheckbox: React.FC<{ checked: boolean; label: string; onToggle: () => void }> = ({
  ...
  >
    ...
    <span className="text-sm font-grotesk text-[var(--imx-text-primary)]">{label}</span>
  </button>
);
...
const Filtres: React.FC = () => {
  ...
  return (
    <div className="page-container pb-24">
      ...
      <header className="bg-[var(--imx-bg-deep)] sticky top-0 z-40 border-b border-[rgba(123,63,228,0.1)] px-4 py-4 flex items-center">
        <button onClick={() => navigate(-1)} className="text-[var(--imx-text-primary)] hover:text-[var(--imx-accent-light)] transition-colors">
          <X size={22} />
        </button>
        <h1 className="section-title text-lg flex-1 text-center pr-6">Filtres</h1>
        <button onClick={handleReset} className="text-sm text-[var(--imx-accent-light)] hover:text-[var(--imx-text-primary)] transition-colors whitespace-nowrap">
          Réinitialiser
        </button>
      </header>
      ...
      <div className="px-4 py-6 space-y-7">
        ...
        <div>
          ...
          <div ref={trackRef} className="relative h-1.5 bg-[var(--imx-surface-2)] rounded-full mx-1 mb-3">
            <div
              className="absolute h-1.5 bg-[var(--imx-accent)] rounded-full"
              style={{
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-grotesk transition-all ${
                  minBedrooms === option.value
                    ? 'bg-[var(--imx-accent)] text-white'
                    : 'bg-[var(--imx-surface-2)] text-[var(--imx-text-secondary)] border border-[rgba(255,255,255,0.1)] hover:border-[var(--imx-accent)]'
                }`}
              >
                {option.label}
              </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[var(--imx-bg-app)] border-t border-[rgba(123,63,228,0.1)] p-4">
