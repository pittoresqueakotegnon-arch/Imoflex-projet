...
const Contact: React.FC = () => {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12" style={{ background: 'var(--imx-bg-app)' }}>
        {/* Icône succès */}
        <div
        ...
        <h1 className="font-nunito font-900 text-2xl text-[var(--imx-text-primary)] text-center mb-3">
          Demande envoyée !
        </h1>
        <p className="text-[var(--imx-text-secondary)] text-sm text-center mb-10 max-w-[280px] leading-relaxed">
    ...
    return (
      <div className="page-container">
        <header className="sticky-header px-4 py-3.5 flex items-center gap-3">
          <BackButton />
          <h1 className="font-nunito font-800 text-lg text-[var(--imx-text-primary)]">Contacter le propriétaire</h1>
        </header>
        <div className="px-4 py-6 space-y-4">
          <div className="h-20 bg-[var(--imx-surface)] rounded-2xl animate-pulse" />
          <div className="h-32 bg-[var(--imx-surface)] rounded-2xl animate-pulse" />
        </div>
      </div>
    );
  }
      ...
      <div className="px-4 py-5 space-y-5 flex-1">
        ...
        {listing && (
          <div className="card p-3 flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl flex-shrink-0 overflow-hidden bg-[var(--imx-surface-2)]">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-nunito font-700 text-[var(--imx-text-primary)] text-sm line-clamp-1">{listing.title}</h3>
              <p className="font-nunito font-900 text-[var(--imx-accent-light)] text-xs mt-0.5">{formatMontant(listing.monthly_rent)} F/mois</p>
            </div>
          </div>
        )}
