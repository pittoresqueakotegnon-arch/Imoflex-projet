    ...
    return (
      <div className="page-container">
        <header className="sticky-header px-4 py-4">
          <h1 className="font-nunito font-800 text-xl text-[var(--imx-text-primary)]">Mes demandes</h1>
        </header>
        <EmptyState
    ...
    return (
      <div className="page-container">
        ...
        <div className="px-4 py-4 space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-28 bg-[var(--imx-surface)] rounded-2xl animate-pulse" />
          ))}
        </div>
        <BottomNav />
      </div>
    ...
    <div className="page-container">
      ...
      <header className="sticky-header px-4 pt-6 pb-3">
        <div className="flex items-center justify-between mb-1">
          <h1 className="font-nunito font-900 text-xl text-[var(--imx-text-primary)]">Mes demandes</h1>
          {pendingCount > 0 && (
            <span
        ...
        <p className="text-[var(--imx-text-secondary)] text-xs mb-4" style={{ fontFamily: 'Space Grotesk' }}>
        ...
        <div className="flex gap-2">
          {(['all', 'nouvelle', 'traitee'] as const).map(s => (
            <button
              ...
              style={{
                background: filter === s ? 'var(--imx-accent)' : 'var(--imx-surface-2)',
                color: filter === s ? '#fff' : 'var(--imx-text-secondary)',
                fontFamily: 'Space Grotesk',
              }}
            >
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: 'var(--imx-surface-2)' }}
                  >
                    {coverPhoto ? (
                      <OptimizedImage
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-0.5">
                      <h3 className="font-nunito font-700 text-[var(--imx-text-primary)] text-sm leading-tight line-clamp-1 flex-1">
                        {listing?.title || 'Annonce supprimée'}
                      </h3>
                      <ChevronRight size={14} className="text-[var(--imx-text-muted)] flex-shrink-0 mt-0.5" />
                    </div>

                    {listing?.monthly_rent && (
                      <div className="font-nunito font-800 text-[var(--imx-accent)] text-sm leading-none mb-1">
                        {formatMontant(listing.monthly_rent)}
                        <span className="text-[10px] font-bold text-[var(--imx-text-muted)] ml-1 uppercase tracking-wide">/ mois</span>
                      </div>
                    )}

                    {listing?.city && (
                      <div className="flex items-center gap-1 mb-1.5">
                        <MapPin size={10} className="text-[var(--imx-text-secondary)]" />
                        <span className="text-[var(--imx-text-secondary)] text-[11px]" style={{ fontFamily: 'Space Grotesk' }}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {/* Date */}
                      <span
                        className="text-[10px] text-[var(--imx-text-muted)]"
                        style={{ fontFamily: 'Space Grotesk' }}
                      >
