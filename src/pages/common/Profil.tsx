...
export default function Profil() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      // Appelle l'Edge Function delete-account (service_role) qui anonymise
      // le profil ET détruit le compte Auth — un simple UPDATE côté client
      // ne suffit pas : l'utilisateur pouvait se reconnecter immédiatement
      // après "suppression" tant que le compte Auth restait actif.
  ...
  return (
    <div className="page-container">
      ...
      <div className="px-4 py-6 space-y-6 flex-1 pb-6">
        <div className="flex flex-col gap-5">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="font-nunito font-800 text-[13px] text-[var(--imx-text-primary)]">Mot de passe</span>
            </button>
            <button
              className="flex-1 py-3 rounded-[14px] flex items-center justify-center gap-2 transition-colors"
              style={{ background: 'var(--imx-surface)', border: '1px solid var(--imx-border)' }}
        ...
        <div>
          ...
          <div className="card divide-y divide-[var(--imx-border)] overflow-hidden">
            {profile.role === 'locataire' ? (
              <>
                <ProfilRow icon={<Home size={16} />} iconColor="#60A5FA" label="Mon logement &amp; loyer" to="/dashboard" />
                <ProfilRow icon={<Receipt size={16} />} iconColor="#22C55E" label="Historique &amp; quittances" to="/historique" />
                <ProfilRow icon={<ClipboardList size={16} />} iconColor="#F59E0B" label="Mes demandes envoyées" to="/mes-demandes" />
                <ProfilRow icon={<Heart size={16} />} iconColor="#EF4444" label="Mes favoris" to="/favoris" />
              </>
            ) : profile.role === 'proprietaire' ? (
              <>
                <ProfilRow icon={<Home size={16} />} iconColor="#60A5FA" label="Mes annonces" to="/pro/annonces" />
                <ProfilRow icon={<ClipboardList size={16} />} iconColor="#F59E0B" label="Demandes reçues" to="/pro/demandes" />
                <ProfilRow icon={<Wallet size={16} />} iconColor="#10B981" label="Mon portefeuille (Wallet)" to="/pro/wallet" />
                <ProfilRow icon={<Heart size={16} />} iconColor="#EF4444" label="Mes favoris" to="/favoris" />
              </>
            ) : null}
          </div>
        ...
        {/* ── Compte & documents ── */}
        <div>
          <SectionLabel>Compte &amp; documents</SectionLabel>
          <div className="card divide-y divide-[var(--imx-border)] overflow-hidden">
            <ProfilRow icon={<Wallet size={16} />} iconColor="#FBBF24" label="Numéro Mobile Money" trailing={mmTrailing} to="/profil/mobile-money" />
          </div>
        </div>
