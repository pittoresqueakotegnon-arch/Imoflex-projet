...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div
      className="fixed inset-0 flex flex-col items-center justify-center z-50 transition-opacity duration-500 bg-[var(--imx-bg-app)]"
      style={{ 
    ...
    >
      ...
      >
        <div className="w-8 h-1.5 rounded-full relative overflow-hidden" style={{ background: 'rgba(139,123,181,0.2)' }}>
          <div className="absolute top-0 left-0 h-full w-full rounded-full animate-pulse" style={{ background: 'var(--imx-accent-light)' }} />
        </div>
      </div>
    </div>
  );
