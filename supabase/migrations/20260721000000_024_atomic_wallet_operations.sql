-- Ajouter la colonne idempotency_key à payments
ALTER TABLE public.payments ADD COLUMN IF NOT EXISTS idempotency_key UUID UNIQUE;

-- RPC pour gérer atomiquement la validation du paiement et la mise à jour des wallets
...
BEGIN
    ...
    -- Idempotence: si déjà traité, on s'arrête là
    ...
    -- Return context pour les notifications côté Deno
    RETURN json_build_object(
        'status', 'approved',
        'payment_id', v_payment.id,
        'owner_id', v_owner_id,
