...
async function createTransaction(params: {
  ...
  // IMPORTANT : maxRetries = 1 (pas de retry automatique) volontairement.
  // POST /transactions CRÉE une nouvelle ressource côté Fedapay et aucune clé
  // d'idempotence n'est envoyée dans la requête. Si on retentait cet appel après
  // un timeout, la transaction Fedapay initiale a pu être créée avec succès malgré
  // la perte de la réponse HTTP côté client : un retry créerait alors une SECONDE
  // transaction distincte pour un seul paiement demandé (double débit potentiel).
  // Un timeout ici doit remonter comme une vraie erreur à traiter manuellement
  // (voir reconcile-payments) plutôt que d'être masqué par un retry silencieux.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // La ligne locale doit exister AVANT tout Push FedaPay : le webhook peut
    // alors toujours retrouver le paiement, même s'il arrive instantanément.
    ...
    try {
      const { token, paymentUrl: url } = await generateToken(fedapayTxId);
      ...
      // Ne pas supprimer ni invalider le paiement : FedaPay peut avoir reçu la
      // demande. La tâche de rapprochement traitera l'état réellement constaté.
      const isUnavailable = fedapayErr instanceof ServiceUnavailableError;
      const msg = fedapayErr instanceof Error ? fedapayErr.message : "Erreur FedaPay";
      console.error(`[circuit-breaker] payment delivery failed (tx=${fedapayTxId}):`, msg);
      return new Response(
        JSON.stringify({
          error: "La demande de paiement est enregistrée mais sa confirmation est en cours. Vérifiez votre historique avant de réessayer.",
