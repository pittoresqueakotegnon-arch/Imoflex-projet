...
// ---- Contrôle d'accès pour les fonctions internes (cron / admin uniquement) ----

/**
 * Vérifie que l'appelant présente bien la clé service_role, et pas un simple JWT
 * utilisateur authentifié valide. `verify_jwt` (config.toml) ne garantit que la
 * validité de la signature du JWT, pas le rôle du porteur : sans ce contrôle,
 * n'importe quel locataire ou propriétaire connecté peut appeler une fonction
 * interne (reconcile-payments, update-overdue-rent-periods, ...).
