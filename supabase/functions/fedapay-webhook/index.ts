  ...
  try {
    ...
    if (eventName === "transaction.approved" || eventName === "transaction.declined") {
      ...
      if (eventName === "payout.sent") {
        ...
        // Le RPC verrouille le retrait et le wallet : un webhook répété ne peut pas
        // recréditer deux fois le même montant.
        const { data: refunded, error: refundError } = await supabase.rpc('fail_withdrawal_and_refund', {
